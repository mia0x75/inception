...
int mysql_cache_one_sql(THD* thd)
{
    ...
    //只允许NOERR, WARNS, RULES, PARSE四种错误级别
...
int mysql_check_inception_variables(
    THD * thd
)
{
    switch(thd->get_stmt_da()->sql_errno())
    {
    case ER_WITH_INSERT_FIELD:
        return inception_check_insert_field ? true : false;

    case ER_NO_WHERE_CONDITION:
        return inception_check_dml_where ? true : false;

    case ER_WITH_LIMIT_CONDITION:
        return inception_check_dml_limit ? true : false;

    case ER_WITH_ORDERBY_CONDITION:
        return inception_check_dml_orderby ? true : false;

    case ER_SELECT_ONLY_STAR:
        return inception_enable_select_star ? false : true;

    case ER_ORDERY_BY_RAND:
        return inception_enable_orderby_rand ? false : true;

    case ER_NOT_ALLOWED_NULLABLE:
        return inception_enable_nullable ? false : true;

    case ER_FOREIGN_KEY:
        return inception_enable_foreign_key ? false : true;

    case ER_USE_TEXT_OR_BLOB:
        return inception_enable_blob_type ? false : true;

    case ER_TABLE_MUST_INNODB:
        return inception_enable_not_innodb ? false : true;

    case ER_PK_COLS_NOT_INT:
        return inception_enable_pk_columns_only_int ? true : false;

    case ER_TABLE_MUST_HAVE_COMMENT:
        return inception_check_table_comment ? true : false;

    case ER_COLUMN_HAVE_NO_COMMENT:
        return inception_check_column_comment ? true : false;

    case ER_TABLE_MUST_HAVE_PK:
        return inception_check_primary_key ? true : false;

    case ER_PARTITION_NOT_ALLOWED:
        return inception_enable_partition_table ? false : true;

    case ER_USE_ENUM:
    case ER_INVALID_DATA_TYPE:
        return inception_enable_enum_set_bit ? false : true;

    case ER_INDEX_NAME_IDX_PREFIX:
    case ER_INDEX_NAME_UNIQ_PREFIX:
        return inception_check_index_prefix ? true : false;

    case ER_AUTOINC_UNSIGNED:
        return inception_enable_autoincrement_unsigned ? true : false;

    case ER_INC_INIT_ERR:
        return inception_check_autoincrement_init_value ? true : false;
    
    case ER_INVALID_IDENT:
        return inception_check_identifier ? true : false;

    case ER_SET_DATA_TYPE_INT_BIGINT:
        return inception_check_autoincrement_datatype ? true : false;

    case ER_TIMESTAMP_DEFAULT:
        return inception_check_timestamp_default ? true : false;

    case ER_CHARSET_ON_COLUMN:
        return inception_enable_column_charset ? false : true;

    case ER_IDENT_USE_KEYWORD:
        return inception_enable_identifer_keyword ? false : true;

    case ER_AUTO_INCR_ID_WARNING:
        return inception_check_autoincrement_name ? true : false;

    case ER_ALTER_TABLE_ONCE:
        return inception_merge_alter_table ? true : false;

    case ER_WITH_DEFAULT_ADD_COLUMN:
        return inception_check_column_default_value ? true : false;
    ...
    while ((item= it++))
    {
        ...
        while (field_node != NULL)
        {
            if (strcasecmp(field_node->field_name, item->full_name()) == 0)
            {
                ...
                {
                    my_error(ER_FIELD_SPECIFIED_TWICE, MYF(0), table_info->table_name, item->full_name());
                    mysql_errmsg_append(thd);
        ...
        if (field_node == NULL)
        {
            my_error(ER_BAD_FIELD_ERROR, MYF(0), table_info->table_name, item->full_name());
            mysql_errmsg_append(thd);
        }
    }
            ...
            while (field_node != NULL)
            {
                if (strcasecmp(field_node->field_name, item->full_name()) == 0)
                {
                    if (!field_node->nullable && 
                        ...
                        item2->is_null())
                    {
                        my_error(ER_BAD_NULL_ERROR, MYF(0), count, field_node->field_name);
                        mysql_errmsg_append(thd);
                    }
...
int mysql_check_create_table(THD *thd)
{
    ...
    {
        my_error(ER_TABLE_CHARSET_MUST_UTF8, MYF(0), create_table->table_name, inception_support_charset);
        mysql_errmsg_append(thd);
    }
...
)
{
    ...
    {
        my_error(ER_WRONG_NAME_FOR_INDEX, MYF(0), table_name, "NULL");
        mysql_errmsg_append(thd);
    }
    ...
    {
        my_error(ER_WRONG_NAME_FOR_INDEX, MYF(0), table_name, key->name.str);
        mysql_errmsg_append(thd);
    }
    ...
    if (key->name.str)
    {
        ...
        {
            if (strncasecmp(key->name.str, "unique_", 7) != 0)
            {
                my_error(ER_INDEX_NAME_UNIQ_PREFIX, MYF(0), table_name, key->name.str);
                mysql_errmsg_append(thd);
            }
        }
        //普通索引匹配 index_
        ...
        {
            if (strncasecmp(key->name.str, "index_", 6) != 0)
            {
                my_error(ER_INDEX_NAME_IDX_PREFIX, MYF(0), table_name, key->name.str);
                mysql_errmsg_append(thd);
            }
        }
    }
    ...
    {
        my_error(ER_TOO_MANY_KEY_PARTS, MYF(0), table_name, key->name.str, inception_max_key_parts);
        mysql_errmsg_append(thd);
    }
}
    ...
    while ((key=key_iterator++))
    {
        ...
        while ((col1= col_it++))
        {
            ...
            while (field_node != NULL)
            {
                if (!strcasecmp(field_node->field_name, col1->field_name.str))
                {
                    ...
                    {
                        my_error(ER_PK_COLS_NOT_INT, MYF(0), table_info->db_name, table_info->table_name, col1->field_name.str);
                        mysql_errmsg_append(thd);
                    }
                    found = TRUE;
                    break;
        ...
        if (!table_info->new_cache)
        {
            ...
            while (source_row)
            {
                ...
                {
                    my_error(ER_DUP_INDEX, MYF(0), table_info->db_name, table_info->table_name, source_row[2]);
                    mysql_errmsg_append(thd);
...
int mysql_field_check(THD* thd, Create_field* field, char* table_name)
{
    ...
    {
        my_error(ER_COLUMN_HAVE_NO_COMMENT, MYF(0), table_name, field->field_name);
        mysql_errmsg_append(thd);
    }
    //对于CHAR类型的长度限制
    ...
    {
        my_error(ER_NOT_ALLOWED_NULLABLE, MYF(0), table_name, field->field_name);
        mysql_errmsg_append(thd);
    }

    if ((field->flags & NOT_NULL_FLAG) && mysql_field_is_blob(field->sql_type))
    {
        my_error(ER_TEXT_NOT_NULLABLE_ERROR, MYF(0), table_name, field->field_name);
        mysql_errmsg_append(thd);
    }
    ...
    {
        my_error(ER_WITH_DEFAULT_ADD_COLUMN, MYF(0), thd->lex->select_lex.table_list.first->table_name, field->field_name);
        mysql_errmsg_append(thd);
    }
...
int mysql_check_add_column(THD *thd)
{
    ...
    while ((field=fields++))
    {
        ...
        if (field->charset && !mysql_field_is_blob(field->sql_type) && (field->flags & BINARY_FLAG == 0))
        {
            my_error(ER_CHARSET_ON_COLUMN, MYF(0), thd->lex->select_lex.table_list.first->table_name, field->field_name);
            mysql_errmsg_append(thd);
        }
...
int mysql_check_alter_option(THD *thd)
{
    ...
    while(create_info.used_fields)
    {
        ...
        else if (create_info.used_fields & HA_CREATE_USED_DEFAULT_CHARSET)
        {
            ...
            {
                my_error(ER_TABLE_CHARSET_MUST_UTF8, MYF(0), thd->lex->query_tables->table_name, inception_support_charset);
                mysql_errmsg_append(thd);
            }
...
int mysql_check_create_db(THD *thd)
{
    ...
    if (create_info_ptr->default_table_charset == NULL ||
        !mysql_check_charset(create_info_ptr->default_table_charset->csname))
    {
        my_error(ER_TABLE_CHARSET_MUST_UTF8, MYF(0), thd->lex->name.str, inception_support_charset);
        mysql_errmsg_append(thd);
    }
